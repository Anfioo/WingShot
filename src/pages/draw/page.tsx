		...
		async (
			...
			capturedSharedBufferRef.current =
				result && "sharedBuffer" in result
					? {
							sharedBuffer: new Uint8ClampedArray(result.sharedBuffer.slice()),
							width: result.width,
	...
	const excuteScreenshot = useCallback(
		async (
			...
			if (
				...
				appInfo(
					"[DIAG] excuteScreenshot: ignored (already capturing or releasing)",
				);
				return;
			}
			...
			if (excuteScreenshotType === ScreenshotType.VideoRecord) {
				appInfo(
					"[DIAG] excuteScreenshot: VideoRecord type, awaiting layer init",
				);
				try {
		...
		async (
			...
			if (imageBufferRef.current && "sharedBuffer" in imageBufferRef.current) {
				...
				if (!pngBuffer && capturedSharedBufferRef.current) {
					appInfo(
						"[DrawPageCore] worker sharedBuffer ref lost, using captured copy",
					);
					pngBuffer =
						await imageLayerActionRef.current?.renderImageSharedBufferToPng(
							capturedSharedBufferRef.current,
