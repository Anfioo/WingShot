    ...
    let (main_window, switch_mouse_through_window) = tokio::join!(
        async {
            match main_window_opt {
                Some(window) => {
                    window.set_always_on_top(true).unwrap();
                    window.set_title("Wing Shot - Full Screen Draw").unwrap();
                    window.show().unwrap();
                ...
                .maximizable(false)
                .minimizable(false)
                .title("Wing Shot - Full Screen Draw")
                .position(0.0, 0.0)
                .inner_size(1.0, 1.0)
                .decorations(false)
                .shadow(false)
        ...
        async {
            match switch_window_opt {
                Some(window) => {
                    window.set_always_on_top(true).unwrap();
                    window
                        .set_title("Wing Shot - Full Screen Draw - Switch Mouse Through")
                        .unwrap();
                    window.show().unwrap();
                ...
                .maximizable(false)
                .minimizable(false)
                .title("Wing Shot - Full Screen Draw - Switch Mouse Through")
                .position(0.0, 0.0)
                .inner_size(1.0, 1.0)
                .decorations(false)
                .shadow(false)
    ...
    let (main_window, toolbar_window) = tokio::join!(
        async {
            match main_window_opt {
                Some(window) => {
                    window.set_always_on_top(true).unwrap();
                    window.set_title("Wing Shot - Video Record").unwrap();
                    window.hide().unwrap();
        ...
        async {
            match toolbar_window_opt {
                Some(window) => {
                    window.set_always_on_top(true).unwrap();
                    window
                        .set_title("Wing Shot - Video Record - Toolbar")
                        .unwrap();
                    window.hide().unwrap();
                ...
                .maximizable(false)
                .minimizable(false)
                .title("Wing Shot - Video Record - Toolbar")
                .position(0.0, 0.0)
                .inner_size(10.0, 10.0)
                .decorations(false)
                .shadow(false)
